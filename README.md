...
| 한글명   | 영문명            | 설명                                                                                                                           |
|-------|----------------|------------------------------------------------------------------------------------------------------------------------------|
| 주문 테이블 | order table    | - 매장에서 관리되는 테이블로 매장 식사 손님용. 매장 식사 손님이 모든 식사를 마치는 경우 테이블을 치운다. <br> - 손님이 테이블 점유 여부(occupied)에 따라 착석 테이블과 빈 테이블 상태로 구분할 수 있다. |
| 착석 테이블 | sitting table  | - 테이블에 손님이 앉아있는 상태를 표현한다.                                                                                                    |
| 빈 테이블 | cleared table  | - 테이블을 치운 상태 또는 아직 손님이 착석하지 않는 상태의 테이블을 표현한다.                                                                                |
| 손님 수  | numberOfGuests | - 착성 테이블 상태 시 손님이 테이블에 앉아있는 수를 의미한다.                                                                                         |
